...
if [ ! -z "$current" ] && [ ! -z "$forecast" ]; then
    ...
    current_city=$(echo "$current" | jq -r ".name") 
    ...
    echo "$(get_icon "$current_icon") $current_temp$SYMBOL  $trend  $(get_icon "$forecast_icon") $forecast_temp$SYMBOL $current_city"
fi
